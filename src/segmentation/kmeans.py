...
def update_centroids(image, labels, k):
    ...
    for i in range(k):
        if np.any(labels_flat == i):
            ...
            # If no pixels are assigned to a cluster, reinitialize the centroid randomly.
...
if __name__ == "__main__":
    ...
    if image is None:
        ...
        k = 5  # Number of clusters (experiment with different values if needed)
        labels, centroids = kmeans_segmentation(image, k)
        
        # Create the segmented image by replacing each pixel with its corresponding centroid color.
        segmented_image = centroids[labels]
